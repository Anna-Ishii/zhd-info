...
function changeMovieUIsize(){
	...
	$('.manualAttachment.isActive').find('.manualAttachment__ui , .manualAttachment__videoCover').css('maxWidth' , modalVideoW);
...
$(document).on('click', '.manualAttachmentBg , .manualAttachment__close' , function(e){
	...
	if($(this).hasClass('manualAttachmentBg') && !e.target.closest('.manualAttachment')){
		...
		$('.manualAttachment.isActive').find('.listPlaySpeed').hide().removeAttr('style');
