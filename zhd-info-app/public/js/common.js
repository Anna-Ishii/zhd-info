...
$(window).on("load", function () {
    ...
    $(document).on(
        "change",
        ".sortMenu__list__item input[type=checkbox]",
        function () {
            targetInput = $(this);
            chkBulk(changeBulk);
        }
    ...
    function chkBulk(callback) {
        var chkTarget = targetInput
            .parents(".sortMenu__list")
            .find("input[type=checkbox]");
        /* 一回一括のチェックを外す */
        chkBulkTarget = targetInput
            .parents(".sortMenu__box")
            .find(".selectAll");
    ...
    $(document).on(
        "click",
        '.modal[data-modal-target="continue"] .modal__close',
        function () {
            var btnModel = clickMessage;
            var message = btnModel.find(".list__item>.list__id").text();
            var editUserListTargetForm = $(
                '.modal[data-modal-target="edit"] form'
            );

            // 初期化
            $(
                '.modal[data-modal-target="edit"] .readEdit__list__accordion li'
            ).remove();
            $(
                '.modal[data-modal-target="edit"] form input[name="message"]'
            ).remove();
            ...
            crewsData.fetchReadCrews(message).then(function () {
                ...
                crewsData.crews.forEach(function (value) {
                    if (value.readed == 0) {
                        $(
                            '.modal[data-modal-target="edit"] .sort_name .readEdit__list__accordion[data-sort-num="' +
                                value.name_sort +
                                '"] ul'
                        ).append(
                            "<li> " +
                                ...
                                " " +
                                value.name +
                                '<input type="checkbox" name="read_edit_radio[]" id="user_' +
                                value.part_code +
                                '_check" data-code="' +
                                value.part_code +
                                '" value="' +
                                ...
                                '"><label for="user_' +
                                value.part_code +
                                '_check" class="readEdit__list__check">未選択</label></li>'
                        );
                    }
                });
                ...
                crewsData.crews.forEach(function (value, index, array) {
                    if (value.readed == 0) {
                        ...
                        if (
                            (count + 1) % maxRow == 0 ||
                            index + 1 == crewsData.crews.length
                        ) {
                            var head =
                                '<div class="readEdit__list__head">' +
                                headFrom_part_code +
                                " ~ " +
                                value.part_code +
                                '</div><div class="readEdit__list__accordion"><ul>' +
                                sortCodeHeader +
                                "</ul></div>";
                            $(
                                '.modal[data-modal-target="edit"] .sort_code'
                            ).append(head);
                            sortCodeHeader = "";
                            if (crewsData.crews[index + 1]) {
                                headFrom_part_code =
                                    crewsData.crews[index + 1].part_code;
                            }
                        }
    ...
    $(document).on("click", ".readUser__switch__item", function () {
        ...
        /* 所属未所属をチェック */
        var chkTabBelongs = $(".readUser__sort")
            .find(".isSelected")
    ...
    $(document).on("click", ".list__status__read", function (e) {
        ...
        crewsData.fetchReadCrews(message).then(function () {
            var readUserListTarget1Element = $(
                '.readUser__list[data-readuser-target="1"]'
            );
            var readUserListTarget2Element = $(
                ...
                if (value.readed == 0) {
                    ...
                    if (value.new_face == 0) {
                        ...
                        readUserListTarget2Element.append(
                            '<li class="readUser__list__item" data-readuser-belong="1"><div><div>' +
                                value.part_code +
                                " " +
                                value.name +
                                "</div><div>" +
                        ...
                        readUserListTarget2Element.append(
                            '<li class="readUser__list__item" data-readuser-belong="2"><div><div>' +
                                value.part_code +
                                " " +
                                value.name +
                                "</div><div>" +
            ...
            var chkTab = $(".readUser__switch__item.isSelected").data(
                "readuser-target"
            );
            if (chkTab == 1) {
                $('button[data-readuser-belong="1"]').text(
    ...
    $(document).on(
        "click",
        '.btnModal[data-modal-target="check"]',
        function (e) {
            e.preventDefault();

            $("#editSort3").prop("checked", true);
            $('.modal[data-modal-target="check"]')
                .find(".readEdit__list.sort_name")
                .show();
            $('.modal[data-modal-target="check"]')
                .find(".readEdit__list.sort_code")
                .hide();
            $('.modal[data-modal-target="check"]')
                .find(".readEdit__list.filter_word")
                .hide();
            ...
            crewsData.fetchCheckCrews().then(function () {
                // 初期化
                $(
                    '.modal[data-modal-target="check"] .readEdit__list__accordion li'
                ).remove();
                $('.modal[data-modal-target="check"] .readEdit__list.sort_code')
                    .find(".readEdit__list__head, .readEdit__list__accordion")
                    .remove();
                $(
                    '.modal[data-modal-target="check"] form input[name="current_url"]'
                ).remove();
                ...
                crewsData.crews.forEach(function (value, index) {
                    // 名前
                    $(
                        '.modal[data-modal-target="check"] .readEdit__list__accordion[data-sort-num="' +
                            value.name_sort +
                            '"] ul'
                    ).append(
                        "<li>" +
                            ...
                            " " +
                            value.name +
                            '<input type="radio" name="read_edit_radio[]" id="user_' +
                            value.part_code +
                            '_radio" data-code="' +
                            value.part_code +
                            '" value="' +
                            ...
                            '"><label for="user_' +
                            value.part_code +
                            '_radio" class="readEdit__list__check">未選択</label></li>'
                    );
                });
                ...
                crewsData.crews.forEach(function (value, index, array) {
                    ...
                    if (
                        ...
                        var head =
                            ...
                            "</ul></div>";
                        $(
                            '.modal[data-modal-target="check"] .sort_code'
                        ).append(head);
                        sortCodeHeader = "";
                        if (crewsData.crews[index + 1]) {
                            headFrom_part_code =
    ...
    $(document).on(
        "click",
        '.btnModal[data-modal-target="logout"]',
        function (e) {
            e.preventDefault();
    ...
    $(document).on(
        "click",
        '.btnModal[data-modal-target="read"]',
        function (e) {
            e.preventDefault();

            $("#editSort1").prop("checked", true);
            $('.modal[data-modal-target="edit"]')
                .find(".readEdit__list.sort_name")
                .show();
            $('.modal[data-modal-target="edit"]')
                .find(".readEdit__list.sort_code")
            ...
            crewsData.fetchReadCrews(message).then(function () {
                ...
                $('.modal[data-modal-target="edit"] .readEdit__list.sort_code')
                    .find(".readEdit__list__head, .readEdit__list__accordion")
                    .remove();
                $(
                    '.modal[data-modal-target="edit"] form input[name="message"]'
                ...
                if (document.getElementById("reading_crews")) {
                    var continueUserListTargetForm = $(
                        '.modal[data-modal-target="continue"] form'
                    );
    ...
    $(document).on(
        "click",
        ".readEdit__list__accordion input[type=checkbox]",
        function () {
            var code = $(this).data("code");

            var check = $(this)
                .parents('div[data-modal-target="edit"]')
                .find('input[data-code="' + code + '"]');
    ...
    $(document).on(
        ...
        function () {
            ...
            var chkTarget = $(this)
                .parents(".readEdit__list")
                .find("input[type=checkbox]:checked");
            var txtReplaceTarget = $(this)
                .parents(".modal__inner")
    ...
    $(document).on(
        "click",
        ".readEdit__list__accordion input[type=radio]",
        function () {
            var code = $(this).data("code");
            $(this)
                .parents('div[data-modal-target="check"]')
                .find(".readEdit__list input")
                .prop("checked", false);
            var checkRadio = $(this)
                .parents('div[data-modal-target="check"]')
                .find('input[data-code="' + code + '"]');
    ...
    $(document).on(
        ...
        ".readEdit__list__accordion input[type=radio]",
        function () {
            var txtResetTarget = $(this)
                .parents(".readEdit")
                .find(".readEdit__list__check");
    ...
    $(document).on("change", "#read_users_sort", function () {
        var targetListHead = $(this)
            .siblings('.readEdit__list:not(".filter_word")')
            .find(".readEdit__list__head");
        if ($(this).is(":checked")) {
            var targetCheck = $(this)
                .siblings(".readEdit__list")
                .find("input[type=checkbox]");
            targetListHead.hide();
    ...
    $(document).on(
        "change",
        ".readEdit__menu__inner input[type=radio]",
        function () {
            var sort_value = $(this).val();
            currnt_sort_value = sort_value;
            var searchText = $(this)
                .parents(".readEdit__menu__inner")
    ...
    $(document).on("input", '.modal input[type="text"]', function () {
        ...
        var readEdit_sortName = $(this)
            .parents(".readEdit")
            .find(".readEdit__list.sort_name");
        var readEdit_sortCode = $(this)
            .parents(".readEdit")
            .find(".readEdit__list.sort_code");
        var readEdit_filterWord = $(this)
            .parents(".readEdit")
            .find(".readEdit__list.filter_word");
                    ...
                    return Object.keys(item).map(function(key) { return item[key]; }).some(function (value, index) {
                        if (value && value.toString().indexOf(kanaFullToHalf(searchText_formatted)) !== -1) {
                            ...
                            li_crew_dom +=
                                ...
                                '><label for="user_' +
                                item.part_code +
                                '_radio" class="readEdit__list__check">' +
                                (isChecked ? "選択" : "未選択") +
                                "</label></li>";
                        }
                    });
    ...
    var crewsData = {
        ...
        fetchCheckCrews: function () {
            ...
            return fetchData(url).done(function (data) {
        ...
        fetchReadCrews: function (message_id) {
            ...
            return fetchData(url, options).done(function (data) {
                crewsData.crews = data.crews;
            });
        },
