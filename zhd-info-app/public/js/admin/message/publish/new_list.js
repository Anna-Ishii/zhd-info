        ...
        $(this).on('click', function() {
            ...
            var newRow = `
                <tr data-message_id="${newMessageId}"
                    data-content_files_list="[]"
                    data-main_file_list="[]"
                    data-organization1_id="2"
                    ...
                    <td nowrap>
                        <p class="messageSaveBtn btn btn-admin" data-message-id="${newMessageId}">保存</p>
                        <p class="messageDeleteBtn btn btn-admin" data-message-id="${newMessageId}">削除</p>
                    </td>
                    ...
                    <td class="label-colum-danger">
                        <div class="emergency-flg-input-group">
                            <input type="checkbox" name="emergency_flg" class="checkCommon mr8"><span>重要</span>
                        </div>
                    </td>
                    <td class="label-category">
                        <div class="category-input-group">
                            <select class="form-control" name="category_id">
                                <option value="1">メニュー・マニュアル関連</option>
                                <option value="2">人事・総務</option>
                                <option value="3">情報共有</option>
                                <option value="4">イレギュラー</option>
                                <option value="5">衛生</option>
                                <option value="6">調査依頼</option>
                            </select>
                        </div>
                    </td>
                    <td class="label-title">
                        <div class="title-input-group" style="display: flex;">
                            ...
                            <input type="button" class="btn btn-admin" id="titleFileNewBtn-${newMessageId}"
                                data-toggle="modal" data-target="#newTitleFileModal-${newMessageId}" value="編集">
                        </div>
                    </td>
                    ...
                    <td class="date-time">
                        <div>
                            <div class="start-datetime-input-group">
                    ...
                    <td class="date-time">
                        <div>
                            <div class="end-datetime-input-group">
                    ...
                    <td style="text-align: right">
                        <div class="shop-edit-group">
                            <input type="button" class="btn btn-admin" id="checkAll-new" name="organizationAll" value="全店">
                            <input type="hidden" id="selectOrganizationAll-new" name="select_organization[all]" value="">

                            <input type="button" class="btn btn-admin" id="shopEditBtn-new"
                                data-toggle="modal" data-target="#editShopModal-new" value="一部">
                        </div>
                    </td>
            ...
            if (!document.getElementById(`newTitleFileModal-${messageId}`)) {
                let fileInputsHtml = '';
            ...
            //             }
                ...
                // 業連ファイル編集処理
                const newTitleFileInputsSelector = `.newTitleFileModal .fileInputs[data-message-id="${messageId}"]`;
                ...
                $(document).on("change", `${newTitleFileInputsSelector} input[type="file"]`, function () {
                    let _this = $(this);
                    ...
                    let filesCount = $(`${newTitleFileInputsSelector} .file-input-container`).length - 1;
                    if (filesCount) {
                ...
                // 削除ボタンのクリックイベント
                $(document).on("click", `${newTitleFileInputsSelector} .delete-btn`, function () {
                    let joinFileBtnAdd = $(`${newTitleFileInputsSelector} .join-file-btn`);
                    let dataCacheCount = $(`${newTitleFileInputsSelector} [data-cache]`).length;
                    let maxFiles = 20; // 上限数を設定（20）
                ...
                // ファイルの結合ボタン処理
                $(document).on("click", `${newTitleFileInputsSelector} #joinFileId-${messageId}`, function () {
                    var selectedFiles = [];
                    var selectedFilePaths = [];
                    var selectedJoinFiles = [];

                    // ファイル名とファイルパスをそれぞれの配列に追加
                    $(`${newTitleFileInputsSelector} [name='file_name[]']`).each(function(){
                        var value = $(this).val();
                        if (value) {
                            selectedFiles.push(value);
                        }
                    });
                    $(`${newTitleFileInputsSelector} [name='file_path[]']`).each(function(){
                        var value = $(this).val();
                        if (value) {
                            selectedFilePaths.push(value);
                        }
                    });
                    $(`${newTitleFileInputsSelector} [name='join_flg[]']`).each(function(){
                        var value = $(this).val();
                        selectedJoinFiles.push(value);
                    });

                    var $modalBody = $(`#newJoinFileModal-${messageId} #fileCheckboxes-${messageId}`);
                    var $modalFooter = $(`#newJoinFileModal-${messageId} .modal-footer`);
                    $modalBody.empty();
                ...
                // 結合ボタン処理
                $(document).on('click', `#newJoinFileModal-${messageId} #joinFileBtn-${messageId}`, function() {
                    // 結合モーダルのチェックされたファイルパスを取得
                    var checkedFileValues = [];
                    $(`#newJoinFileModal-${messageId} #fileCheckboxes-${messageId} input[type="checkbox"]:checked`).each(function() {
                    ...
                    $(`${newTitleFileInputsSelector} [name='file_path[]']`).each(function(index) {
                        var value = $(this).val();
                        if (checkedFileValues.includes(value)) {
                            $(`${newTitleFileInputsSelector} [name='join_flg[]']`).eq(index).val("join");
                            // 結合ラベルを表示
                            $(this).closest('.row').find("label[style*='padding-top: 10px']").show();
                        } else {
                            $(`${newTitleFileInputsSelector} [name='join_flg[]']`).eq(index).val("single");
                            // 結合ラベルを非表示
                            $(this).closest('.row').find("label[style*='padding-top: 10px']").hide();
                        }
                    });

                    var modalFooterMessage = $(`${newTitleFileInputsSelector} .join-file-btn .inputFile p`);
                    ...
                    $(`#newJoinFileModal-${messageId}`).modal("hide");
                });

                // 結合モーダルのチェックボックス変更イベント処理
                $(document).on('change', `#newJoinFileModal-${messageId} #fileCheckboxes-${messageId} input[type="checkbox"]`, function() {
                    // 選択されたファイルのカウントを更新する関数
                    updateJoinFileCount(messageId);
                });
                ...
                $(document).on('click', `#fileImportBtn-${messageId}`, function() {
                    ...
                    // モーダルを閉じる
                    $(`#newTitleFileModal-${messageId}`).modal("hide");
                });
            ...
            function handleResponse(response, fileName, filePath, joinFile, dataCache, messageId) {
                ...
                if (!dataCache) {
                    let fileInputs = document.querySelector(`.newTitleFileModal .fileInputs[data-message-id="${messageId}"]`);
                    ...
                    // 上限を超えていない場合、かつファイル数が上限に達していない場合のみファイル入力欄を追加
                    let existingFilesCount = $(`.newTitleFileModal .fileInputs[data-message-id="${messageId}"] .file-input-container`).length;
                    let joinFileBtnAdd = $(`.newTitleFileModal .fileInputs[data-message-id="${messageId}"] .join-file-btn`);
                ...
                // PDFファイルの上書き
                    ...
                    $(`.newTitleFileModal .fileInputs[data-message-id="${messageId}"] [name='join_flg[]']`).each(function() {
                        if ($(this).val() === "single") {
                            // 結合ラベルを非表示
                            $(this).closest('.row').find("label[style*='padding-top: 10px']").hide();
                        }
            ...
            function addNewFileInput(content_name, content_url, join_flg, messageId) {
                // 既存の添付ラベルの数を取得
                let currentLabelCount = $(`.newTitleFileModal .fileInputs[data-message-id="${messageId}"] .file-input-container .control-label:contains('添付')`).length + 1;

                $(`.newTitleFileModal .fileInputs[data-message-id="${messageId}"]`).append(`
                    <div class="file-input-container">
                        <div class="row">
            ...
            function renumberSendLabels(messageId) {
                $(`.newTitleFileModal .fileInputs[data-message-id="${messageId}"] .file-input-container .control-label:contains('添付'), .newTitleFileModal .fileInputs[data-message-id="${messageId}"] .file-input-container .control-label:contains('業連')`).each(function(index) {
                    if (index === 0) {
            ...
            function updateJoinFileCount(messageId) {
                var checkedCount = $(`#newJoinFileModal-${messageId} #fileCheckboxes-${messageId} input[type="checkbox"]:checked`).length;

                // 既存のメッセージを削除
                $(`#newJoinFileModal-${messageId} .modal-footer p`).remove();

                // メッセージを追加
                if (checkedCount >= 2) {
                    $(`#newJoinFileModal-${messageId} .modal-footer`).append(`<p style="float: left;">${checkedCount}ファイルを結合します。よろしいでしょうか？</p>`);
                } else if (checkedCount == 0) {
                    $(`#newJoinFileModal-${messageId} .modal-footer`).append(`<p style="float: left;">結合するファイルが選択されていません。</p>`);
                }

                // ボタンの有効/無効を設定
                var modalFooterJoinFileBtn = $(`#newJoinFileModal-${messageId} .modal-footer #joinFileBtn-${messageId}`);
            ...
            function updateModalFooterMessage(messageId) {
                ...
                var modalFooterMessage = $(`.newTitleFileModal .fileInputs[data-message-id="${messageId}"] .join-file-btn .inputFile p`);
                if (modalFooterMessage.length) {
                    if (checkedCount >= 2) {
                        modalFooterMessage.text(`${checkedCount}ファイルを結合します。`).show();
                    } else {
            ...
            function updateJoinFileLabel(messageId) {
                ...
                var joinFlagCount = $(`.newTitleFileModal .fileInputs[data-message-id="${messageId}"] [name='join_flg[]']`).filter(function() {
                ...
                if (joinFlagCount <= 1) {
                    ...
                    $(`.newTitleFileModal .fileInputs[data-message-id="${messageId}"] .inputFile #joinFileId-${messageId}`).val("ファイルの結合");
                }
                ...
                if (hasJoinFlag) {
                    $(`.newTitleFileModal .fileInputs[data-message-id="${messageId}"] .inputFile #joinFileId-${messageId}`).val("結合の修正");
                }
            }
            ...
            function updateSelectedStores(messageId) {
                const selectedCount = $(`.newShopSelectModal .shopSelectInputs[data-message-id="${messageId}"] input[name="organization_shops[]"]:checked`).length;
                $(`.newShopSelectModal .shopSelectInputs[data-message-id="${messageId}"] .storeSelected`).text(`${selectedCount}店舗選択中`);
            }

            // チェックボックスの連携を設定
            function syncCheckboxes(storeId, checked, messageId) {
                document.querySelectorAll(`.newShopSelectModal .shopSelectInputs[data-message-id="${messageId}"] input[data-store-id="${storeId}"]`).forEach(function(checkbox) {
                    checkbox.checked = checked;
                });

                // 各親組織のチェックボックスを更新
                const organizationId = document.querySelector(`.newShopSelectModal .shopSelectInputs[data-message-id="${messageId}"] input[data-store-id="${storeId}"]`).getAttribute('data-organization-id');
                if (organizationId) {
            ...
            function updateParentCheckbox(messageId, organizationId) {
                const parentCheckbox = document.querySelector(`.newShopSelectModal .shopSelectInputs[data-message-id="${messageId}"] input[data-organization-id="${organizationId}"]`);
                if (parentCheckbox) {
                    const childCheckboxes = document.querySelectorAll(`.newShopSelectModal .shopSelectInputs[data-message-id="${messageId}"] input[data-organization-id="${organizationId}"].shop-checkbox`);
            ...
            function updateAllParentCheckboxes(messageId) {
                const parentCheckboxes = document.querySelectorAll(`.newShopSelectModal .shopSelectInputs[data-message-id="${messageId}"] input.org-checkbox`);
            ...
            function updateSelectAllCheckboxes(messageId) {
                // 組織タブのチェックボックスの状態を更新
                const organizationCheckboxes = $(`.newShopSelectModal .shopSelectInputs[data-message-id="${messageId}"] #byOrganization-${messageId} input.shop-checkbox`);
                const selectAllOrganizationCheckbox = $(`.newShopSelectModal .shopSelectInputs[data-message-id="${messageId}"] #selectAllOrganization-${messageId}`);
                ...
                const storeCodeCheckboxes = $(`.newShopSelectModal .shopSelectInputs[data-message-id="${messageId}"] #byStoreCode-${messageId} input.shop-checkbox`);
                const selectAllStoreCodeCheckbox = $(`.newShopSelectModal .shopSelectInputs[data-message-id="${messageId}"] #selectAllStoreCode-${messageId}`);
            ...
            function removeSelectedClass(messageId) {
                ...
                $(`.newShopSelectModal .shopSelectInputs[data-message-id="${messageId}"] .check-store-list .btn`).removeClass("check-selected");
