...
Options:
	...
	-e
	--enc       Name of the encoding table to use. Leave empty for default mode. 
				Omit this parameter for TrueType Unicode and symbolic fonts like Symbol or ZapfDingBats.

	-f
	--flags     Unsigned 32-bit integer containing flags specifying various characteristics of the font 
				(PDF32000:2008 - 9.8.2 Font Descriptor Flags): 
				+1 for fixed font; 
				+4 for symbol or +32 for non-symbol; 
				+64 for italic. Fixed and Italic mode are generally autodetected so you have to set it to 32 = non-symbolic font (default) or 4 = symbolic font.

	-o
	--outpath   Output path for generated font files (must be writeable by the web server). Leave empty for default font folder.

	-p
	--platid    Platform ID for CMAP table to extract (when building a Unicode font for Windows this value should be 3, for Macintosh should be 1).

	-n
	--encid     Encoding ID for CMAP table to extract (when building a Unicode font for Windows this value should be 1, for Macintosh should be 0). 
	            When Platform ID is 3, legal values for Encoding ID are: 0=Symbol, 1=Unicode, 2=ShiftJIS, 3=PRC, 4=Big5, 5=Wansung, 6=Johab, 7=Reserved, 8=Reserved, 9=Reserved, 10=UCS-4.
				抽出するCMAPテーブルのエンコーディングID（Windows用のUnicodeフォントを作成する場合、この値は1、Macintoshの場合は0にする必要があります）。
				プラットフォームIDが3の場合、エンコーディングIDの有効な値は次のとおりです：0=Symbol, 1=Unicode, 2=ShiftJIS, 3=PRC, 4=Big5, 5=Wansung, 6=Johab, 7=Reserved, 8=Reserved, 9=Reserved, 10=UCS-4.
	-b
	--addcbbox  Includes the character bounding box information on the php font file.
				phpフォントファイルに文字バウンディングボックス情報が含まれています。
