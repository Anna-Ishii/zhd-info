                            ...
                            @if (session('check_crew'))
                                <div>
                                    <input type="checkbox" id="not_read_check" name="not_read_check" value="1"
                                        {{ request()->input('not_read_check') == 1 ? 'checked' : '' }}>
                ...
                <div class="list__inner">
                    ...
                    @foreach ($messages as $message)
                        <a href="" class="btnModal" data-modal-target="read">
                            <div
                                class="list__item {{ session('check_crew') && isset($message->readed_crew_count) && $message->readed_crew_count != 0 ? 'readed' : '' }}">
                                ...
                                <div class="list__status">
                                    <div class="list__status__limit">
                                        {{ $message->start_datetime?->isoFormat('MM/DD') }}〜{{ $message->end_datetime?->isoFormat('MM/DD') }}
                                    </div>
                                    <div class="list__status__read">{{ $message->view_rate }}%(
                                        {{ $message->readed_crew_count }}/ {{ $message->crew_count }})</div>
                                </div>
                            </div>
                        </a>
                    @endforeach
