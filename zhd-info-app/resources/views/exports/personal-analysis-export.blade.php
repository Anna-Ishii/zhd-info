<table class="personal table">
    <thead>
        <tr style="line-height: 12px; text-align: center;">
            ...
            @endforeach
            <th class="head1" colspan="2" style="background-color: #bbbbbb;">
                <div>店舗</div>
            </th>
            <th class="head1" colspan="2" style="background-color: #bbbbbb;">
                <div>期間計</div>
            ...
            @foreach ($messages as $m)
                <th class="head2" colspan="2" style="background-color: #d1daef;">
    ...
    <tbody>
        <tr style="background-color: #ffffff;">
            <td colspan="{{ count($organizations) + 2 }}" style="border-bottom: 1px solid black;">{{ $organization1->name }}計</td>
            <td style="border-bottom: 1px solid black; text-align: right;">{{ $viewrates['org1_readed_sum'] ?? 0 }} / {{ $viewrates['org1_sum'] ?? 0 }}</td>
            ...
            @isset($viewrates[$organization][0])
                ...
                @foreach ($viewrates[$organization][0] as $v_org_key => $v_o)
                    <tr style="{{ $isEven($index) ? 'background-color: #d3d3d3;' : '' }}">
                        <td colspan="{{ count($organizations) + 2 }}" style="border-bottom: 1px solid black;">{{ $v_o->name }}</td>
                        <td style="border-bottom: 1px solid black; text-align: right;">
    ...
    <tbody>
        @isset($viewrates['shop'][0])
            ...
            @foreach ($viewrates['shop'][0] as $v_key => $m_c)
                <tr style="{{ $isEven($index) ? 'background-color: #d3d3d3;' : '' }}">
                    @isset($m_c->o3_name)
                        <td class="orgDS" style="border-bottom: 1px solid black;">{{ $m_c->o3_name }}</td>
                    @endisset
                    @isset($m_c->o4_name)
                        <td class="orgAR" style="border-bottom: 1px solid black;">{{ $m_c->o4_name }}</td>
                    @endisset
                    @isset($m_c->o5_name)
                        <td class="orgBL" style="border-bottom: 1px solid black;">{{ $m_c->o5_name }}</td>
                    @endisset
                    <td style="border-bottom: 1px solid black; text-align: center;">{{ $m_c->shop_code }}</td>
                    <td style="border-bottom: 1px solid black; text-align: left;">{{ Str::limit($m_c->shop_name, 12) }}</td>
                    <td style="border-bottom: 1px solid black; text-align: right;">
                    ...
                    @foreach ($messages as $key => $ms)
                        @isset($viewrates['shop'][$key][$v_key]->count)
                            ...
                            <td data-message={{ $ms->id }} style="border-bottom: 1px solid black; text-align: right;">
