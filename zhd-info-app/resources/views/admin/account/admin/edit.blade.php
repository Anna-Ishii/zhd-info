            ...
            <div class="form-group">
                ...
                <div class="col-lg-4">
                    <input class="form-control" name="employee_code"
                        value="{{ old('employee_code', $edit_admin->employee_code) }}">
                </div>
            </div>
            <div class="form-group">
                    ...
                    <div class="mb8">
                        <label class="mr16">
                            <input type="checkbox" id="checkAll" class="mr8"
                                @if (request()->old()) {{ old('organization1', []) == $organization1_list->pluck('id')->toArray() ? 'checked' : '' }}
                            @else
                                {{ empty(array_diff($organization1_list->pluck('id')->toArray(), $edit_admin->organization1->pluck('id')->toArray())) ? 'checked' : '' }} @endif>
                            全て
                        </label>
                    </div>
                    @foreach ($organization1_list as $organization1)
                        <label class="mr16">
                            <input type="checkbox" name="organization1[]" value="{{ $organization1->id }}"
                                class="checkCommon mr8"
                                @if (request()->old()) {{ in_array((string) $organization1->id, old('organization1', []), true) ? 'checked' : '' }}
                        @else
                            {{ in_array($organization1->id, $edit_admin->organization1->pluck('id')->toArray(), true) ? 'checked' : '' }} @endif>
                            {{ $organization1->name }}
                        </label>
                    @endforeach
                </div>
                    ...
                    <select name="ability" class="form-control">
                        @foreach ($ability_list as $ability)
                            <option value="{{ $ability->value }}" class="mr8"
                                @if (request()->old()) {{ $ability->value == old('ability') ? 'selected' : '' }}
                            @else
                                {{ $ability->value == $edit_admin->ability->value ? 'selected' : '' }} @endif>
                                {{ $ability->text() }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="form-group">
                ...
                <div class="col-lg-10 checkArea">
                    <div class="mb8">
                        <label class="mr16">
                            <input type="checkbox" id="checkAll" class="mr8"
                                @if (request()->old()) {{ old('page', []) == $adminpage_list->pluck('id')->toArray() ? 'checked' : '' }}
                            @else
                                {{ $edit_admin->allowpage->pluck('id')->toArray() == $adminpage_list->pluck('id')->toArray() ? 'checked' : '' }} @endif>
                            全て
                        </label>
                    </div>
                    @foreach ($adminpage_list as $page)
                        <label class="mr16">
                            <input type="checkbox" name="page[]" value="{{ $page->id }}" class="checkCommon mr8"
                                @if (request()->old()) {{ in_array((string) $page->id, old('page', []), true) ? 'checked' : '' }}
                        @else
                            {{ in_array($page->id, $edit_admin->allowpage->pluck('id')->toArray(), true) ? 'checked' : '' }} @endif>
                            {{ $page->name }}
                        </label>
                    @endforeach
                </div>
                ...
                <div class="col-lg-10 checkArea">
                    <label class="mr16">
                        <input type="radio" name="is_valid" value="true" class="mr8"
                            @if (request()->old()) {{ !old('is_valid') ? '' : 'checked' }}
                        @else
                            {{ $edit_admin->trashed() ? '' : 'checked' }} @endif>
                        有効
                    </label>
                    <label class="mr16">
