...
class WowTalkNotificationSenderCommand extends Command
{
    ...
    private function sendNotifications()
    {
        ...
        // 現在掲載中と掲載終了を取得
        $messages = Message::where('editing_flg', false)->where('is_broadcast_notification', true)->get();
    ...
    private function sendWowTalkMessages($message)
    {
        ...
        foreach ($wowtalk_data as $data) {
            try {
                // メッセージ内容を生成
                $messageContent = $this->calculateAndGenerateMessageContent($message);
                ...
                if (!empty($data[$wowtalkIdKey])) {
                    try {
                        ...
                        if (is_array($apiResult)) {
                            $errorLogs[] = $this->createErrorLog($message, $data, $data[$wowtalkIdKey], $apiResult, $messageContent);
                        }
                    } catch (\Exception $e) {
                        // エラーログを生成
                        $errorLogs[] = $this->createErrorLog($message, $data, $data[$wowtalkIdKey], $apiResult, $messageContent);
     ...
     */
    private function calculateAndGenerateMessageContent($message)
    {
        // メッセージ内容を生成
        $messageContent = "業連が配信されました。確認してください。\n";
        $messageContent .= "・業連名：{$message->title}\n";
    ...
    {
        // 業連配信通知フラグを更新
        $message->is_broadcast_notification = false;
        $message->save();
